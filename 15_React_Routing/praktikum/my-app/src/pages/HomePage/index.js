  ...
  return (
    <>
      ...
      <div className="body">
        <br />
        <h1>todos</h1>
        <div className="row row-form justify-content-between border">
            ...
            .map((task, index) => {
              return (
                <React.Fragment key={task.id}>
                  {/* <= keyprops */}
                  <div className="col taskDecor border-top-0 border-bottom border-dark">
                    <Input
                      type="checkbox" //checklist to define changes in status:id
                      onClick={() => markDoneTask(task.id)}
                      className="check-box"
                    />
                    <div
                      className={task.status ? "done fst-italic" : "not-done"}
                    >
